                    ...
                    <div class="row g-4">
                        <div class="col-lg-7">
                            <div class="card">
                                ...
                                <div class="card-body">
                                    <%-- Normalize license URL: DB may store /ctx/uploads/..., /uploads/..., or uploads/... --%>
                                    <c:if test="${not empty restaurant.licenseFileUrl}">
                                        <c:set var="licRaw" value="${fn:trim(restaurant.licenseFileUrl)}" />
                                        <c:set var="ctx" value="${pageContext.request.contextPath}" />
                                        <c:choose>
                                            <c:when test="${fn:startsWith(licRaw, 'http://') || fn:startsWith(licRaw, 'https://')}">
                                                <c:set var="licenseHref" value="${licRaw}" />
                                            </c:when>
                                            <c:when test="${not empty ctx && fn:startsWith(licRaw, ctx.concat('/'))}">
                                                <c:set var="licenseHref" value="${licRaw}" />
                                            </c:when>
                                            <c:when test="${fn:startsWith(licRaw, '/')}">
                                                <c:set var="licenseHref" value="${ctx}${licRaw}" />
                                            </c:when>
                                            <c:otherwise>
                                                <c:set var="licenseHref" value="${ctx}/${licRaw}" />
                                            </c:otherwise>
                                    ...
                                    <div class="row mb-3">
                                        ...
                                        <div class="col-md-8">
                                            <c:choose>
                                                <c:when test="${not empty restaurant.licenseFileUrl}">
                                                    <a href="${licenseHref}" target="_blank" rel="noopener noreferrer" class="btn btn-sm btn-outline-primary">
