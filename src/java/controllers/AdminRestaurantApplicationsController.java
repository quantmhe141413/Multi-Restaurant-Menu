            ...
            if (ok) {
                ...
                try {
                    if (restaurant.getOwnerEmail() == null || restaurant.getOwnerEmail().trim().isEmpty()) {
                        ...
                        if (!emailSent) {
                            emailError = "Email service failed to send notification";
                        }
                    }
                } catch (Exception e) {
                    emailError = "Unexpected error while sending email: " + e.getMessage();
                    e.printStackTrace();
                }
                
                String message = "Restaurant status updated successfully";
                if (emailSent) {
                    message += " and notification email sent to owner";
                } else if (emailError != null) {
                    message += " but email notification failed: " + emailError;
                } else {
                    message += " but email notification was not sent";
                }
